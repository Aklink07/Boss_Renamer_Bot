...
# --- FLASK SERVER (For UptimeRobot & Render Port Binding) ---
...
async def progress(current, total, message, start_time):
    ...
    if round(diff % 5.00) == 0 or current == total:
        ...
        try: await message.edit_text(f"**Uploading:** {round(percentage, 2)}% ⬆️")
        except: pass

# --- HANDLERS ---
@app.on_message(filters.command("start"))
async def start(client, message):
    await message.reply_text(f"👋 Hello {message.from_user.first_name}! I am organized and ready.")
...
async def rename_file(client, message):
    ...
    try:
        ...
        return await message.reply("❌ Usage: `/rename filename.mkv`")
...
# --- MAIN EXECUTION (Python 3.12/3.14 Friendly) ---
async def start_bot():
...
if __name__ == "__main__":
    try:
        # Create and run new event loop to avoid 'no current event loop' error
